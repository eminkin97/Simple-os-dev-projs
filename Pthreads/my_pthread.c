...
int my_pthread_create(my_pthread_t * thread, pthread_attr_t * attr, void *(*function)(void*), void * arg) {
	...
	if (priority_Queue == NULL ) {
	    ...
	    return_link.uc_stack.ss_sp = malloc(FIBER_STACK);
		...
		priority_Queue = (mpq *) malloc(sizeof(mpq));		//allocate priority queue
		...
		thread_count_mtx = (my_pthread_mutex_t *)malloc(sizeof(my_pthread_mutex_t));
	...
	// Modify the context to a new stack
    	...
    	newthread->uc_stack.ss_sp = malloc(FIBER_STACK);
	...
	*thread = thread_count1;
	...
	tcb_list *newNode = (tcb_list *) malloc(sizeof(tcb_list));
...
void scheduler(int ranWholeTimeSlice) {
	...
	if (running_queue_head == NULL) {
		...
		ucontext_t *originalthread = (ucontext_t *) malloc(sizeof(ucontext_t));
		my_pthread_t ot = 1;
		...
		if (ptr -> data -> status == THREAD_EXITED) {		//if thread exited
			...
			free(ptr -> data -> thread_context -> uc_stack.ss_sp);	//free stack pointer
			free(ptr -> data);		//free thread control block
			free(ptr);				//free list node
	...
	if (exited == 1) {
		...
		setcontext(running_queue_head -> data -> thread_context);
	} else {
		//Swapping Context
		...
		//swapcontext(&currentthread, running_queue_head -> data -> thread_context);
...
void my_pthread_exit(void *value_ptr) {
	...
	//deal with retval
	if (value_ptr != NULL) {
		retval_elem *val = (retval_elem *)malloc(sizeof(retval_elem));
		val -> thread_id = ptr -> data -> thread_id;
		val -> retval = value_ptr;

		retval_block *block = (retval_block *)malloc(sizeof(retval_block));
		block -> data = val;
...
int my_pthread_join(my_pthread_t thread, void **value_ptr) {
	...
	join_var *cond_var = (join_var *) malloc(sizeof(join_var));
	cond_var -> done = 0;		//if thread we are waiting for is done
	cond_var -> thread_waiting_for = thread;

	join_var_block *block = (join_var_block *) malloc(sizeof(join_var_block));
	block -> data = cond_var;
